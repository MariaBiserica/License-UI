...
class _HelpAndDocumentationWidgetState extends State<HelpAndDocumentationWidget> {
  ...
  final Map<String, List<String>> algorithmDetails = {
    'VGG16': [
      ...
      'The classic architecture of VGG16 consists of 16 layers, including 13 convolutional layers and 3 fully connected layers. Each convolutional layer uses 3x3 filters, followed by a 2x2 max-pooling layer after each set of convolutional layers. The final network structure includes: \n * 2 convolutional layers (64 filters) + max-pooling \n * 2 convolutional layers (128 filters) + max-pooling \n * 3 convolutional layers (256 filters) + max-pooling \n * 3 convolutional layers (512 filters) + max-pooling \n * 3 convolutional layers (512 filters) + max-pooling \n * 3 fully connected layers + softmax for classification.',
      'Adapting VGG16 for regression tasks offers several advantages:\n* Transfer Learning: Pre-trained weights from large datasets like ImageNet can be used to extract relevant features from images, reducing training time and resources.\n* Accuracy: VGG16s deep architecture and small convolutional layers (3x3) capture fine details at each depth level, contributing to high accuracy.\n* Flexibility: VGG16 can be easily adapted to different data types and tasks by modifying the output layers.\n* Reduced Overfitting: Transfer learning and built-in regularization help reduce overfitting, improving model generalization on unseen data.\n* Computational Efficiency: Despite its depth, VGG16’s simplified structure and efficient use of convolutions and max-pooling make training and inference faster compared to other deep networks.',
