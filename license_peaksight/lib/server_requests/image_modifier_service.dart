...
Future<String?> applySharpening(String imagePath, int kernelSize) async {
  try {
    var uri = Uri.parse('http://127.0.0.1:5000/apply_sharpening');
    var request = http.MultipartRequest('POST', uri)
      ..files.add(await http.MultipartFile.fromPath('image', imagePath))
      ..fields['kernel_size'] = kernelSize.toString();
    ...
    if (response.statusCode == 200) {
      var responseData = await response.stream.toBytes();
      var originalFileName = path.basenameWithoutExtension(imagePath);
      var filePath = '${Directory.systemTemp.path}/${originalFileName}_sharpened_${DateTime.now().millisecondsSinceEpoch}.jpg';
      var file = File(filePath);
      await file.writeAsBytes(responseData);
      return filePath;
    } else {
